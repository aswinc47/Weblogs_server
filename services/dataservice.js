...
updateBlog = (title,category,description,blogContent,image,id)=>{
    return db.Post.findOne({_id:id}).then(data=>{
        if(data){
            return db.Post.updateOne({_id:id},{$set:{title:title,category:category,description:description,blogContent:blogContent,image:image}}).then(data=>{
                if(data){
                    return{
                        status:true,
                        statusCode:200,
...
deleteAccount = (email) => {
  return db.User.findOne({ email }).then((user) => {
    if (user) {
      return db.User.deleteOne({ email }).then(() => {
        return db.Post.deleteMany({ email }).then(() => {
          console.log("here");
          return {
            statusCode: 200,
            status: true,
